...
- `bffnt.py` — розпаковує шрифти у читабельний набір PNG + `font.json` і може пакувати назад (байт-в-байт із оригіналу).
...
- Пакування назад (байт‑у‑байт з `file_b64` з `font.json`):
...
- Запаковка (режим `pack`):
  - Це байт‑у‑байт “repack”: записує назад вказаний у `font.json` `file_b64` без реконструкції структур.
...
- `file_b64` — сирий BFFNT у base64 (для біт‑ідентичного repack).
